...
Covers:
  ...
  2. `assess_lead` / `regenerate_draft` -- the resolved instruction reaches
     ...
     preserved untranslated and the sign-off stays a generic "Raed Ventures"
     with no individual name (issue #137).
"""
...
class _RoleplayingCompletions:
    ...
    language, echoing the Calendly URL verbatim and signing off generically
    as "Raed Ventures" -- exactly as we instruct the real model to do."""
...
def test_assess_lead_arabic_applicant_yields_arabic_draft(monkeypatch):
    ...
    # Sign-off is a generic "Raed Ventures" -- no individual owner name.
