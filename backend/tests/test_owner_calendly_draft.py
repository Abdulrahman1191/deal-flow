...
single hardcoded default (issue #84). The sign-off itself no longer carries
any personal/associate name — since all outreach sends from the unified
submission@raed.vc / "Raed Ventures" identity, drafts sign off as "Raed
Ventures" only regardless of who the lead owner is (issue #136). Covers
three layers:

  1. claude_agent.assess_lead / regenerate_draft — the actual prompt
     building, exercised against a fake DeepSeek client (mirrors the
...
class _EchoingCompletions:
    ...
    Calendly link appears in the prompt it was given, and always signs off
    as just "Raed Ventures", matching how the real model is instructed to
    behave (issue #136 -- no personal/associate name in the closing)."""
...
def test_regenerate_draft_uses_owner_calendly_and_omits_owner_name(monkeypatch):
...
def test_assess_lead_uses_owner_calendly_and_omits_owner_name(monkeypatch):
    _install_fake_llm(monkeypatch)
    lead_data = {
