...
_LANGUAGE_INSTRUCTIONS = {
    "ar": (
        ...
        'translate or alter the Calendly URL itself, and keep "Raed Ventures" '
        "exactly as given, untransliterated."
    ),
    "en": (
...
- If YES: draft_type "meeting_request". Short, plain, warm email, MAX 50 WORDS, no
  ...
  {calendly_url}. Sign off as just "Raed Ventures" — never a personal or
  associate name. Follow this shape:
    Hi {{name}},
...
- If REJECT: draft_type "rejection". Short, honest, respectful email, MAX 70 WORDS,
  ...
  just "Raed Ventures" — never a personal or associate name.
...
def assess_lead(
    ...
    draft's meeting link is the owner's, not a single hardcoded default.
    Callers without an owner on hand may omit it; generation falls back to
    DEFAULT_CALENDLY_URL so it never breaks.

    `owner_name` is accepted for backward compatibility with existing callers
    but is no longer used: since all outreach now sends from the unified
    submission@raed.vc / "Raed Ventures" identity, drafts sign off as "Raed
    Ventures" only, with no personal name (issue #136).
    """
...
- REJECT: draft_type MUST be "rejection". Short, honest, respectful email,
  ...
  focus, model type). Sign off as just "Raed Ventures" — never a personal or
  associate name.
